...
from loss_comb import *
from resnet_comb import resnet_comb
...
root_1 = current_directory + "/log/res_comb_mli_imb.log"
root_2 = current_directory + "/log/res_comb_mli_imb.log_file"
...
parser = argparse.ArgumentParser(description='Malimg Classification')
parser.add_argument('--dataset', default='Malimg', type=str,
                    help='dataset (Malevis or Malimg[default])')
...
parser.add_argument('--imb_factor', type=float, default=0)
...
# 参数
parser.add_argument('--alpha', default=0.01, type=float, help='[0.25, 0.5, 0.75, 1.0,1.5]')
parser.add_argument('--beta', default=0.25, type=float, help='[0.25, 0.5, 0.75, 1.0,1.5]')
parser.add_argument('--head', default=3, type=int, help='[10, 20, 30, 40]')
...
parser.add_argument('--model', default='resnet50', type=str,
                    help='Model to be trained. '
                         'Select from resnet{18, 34, 50, 101, 152, 50_v2} / resnext{50_32x4d, 101_32x8d} / '
...
best_F11 = 0
best_F11_mi = 0
kg = torch.zeros(25, 25)
feature_mean = torch.zeros(25, 4096)  # 64
...
def main():
    ...
    # loss.py
    criterion = FSDA_CE(feature_num, 25, cls_num_list=img_num_list,
                         max_m=0.5, s=30)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if epoch < 80:
            ...
            prec1_train, _, _, _, _, _, _, preds_train, labels_train, cf_normalized = validate(imbalanced_train_loader, model, nn.CrossEntropyLoss().cuda(), epoch)
    ...
    print('Best accuracy:', best_prec1, 'Best Recall:', best_Recall1, 'Best Precision:', best_Precision1, 'Best F1:',
          best_F11)
    print('微平均: Best Recall:', best_Recall1_mi,
          'Best Precision:', best_Precision1_mi, 'Best F1:', best_F11_mi)
...
def train_FSDA(train_loader, model, optimizer_a, epoch, criterion, alpha, kg, beta, out_new, feature_mean, args):
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        cls_loss = criterion(model.fc, features, predicts, target_var, alpha, weights, cv_var, "update", kg,
...
def get_feature_mean(imbalanced_train_loader, model, class_num):
    model.eval()
    feature_mean_end = torch.zeros(class_num, 4096)
    with torch.no_grad():

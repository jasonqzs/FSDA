...
class EstimatorCV():
    def __init__(self, feature_num, class_num):
        ...
        self.CoVariance = torch.zeros(class_num, feature_num, feature_num).cuda()  # 协方差矩阵
        self.Ave = torch.zeros(class_num, feature_num).cuda()  # 用于存储每个类别的特征均值
        self.Amount = torch.zeros(class_num).cuda()  # 这个张量用于存储每个类别的样本数量
...
class FSDA_CE(nn.Module):
    ...
    def FSDA(self, fc_kg_new, features, y_s, labels_s, s_cv_matrix, alpha, kg, out_new, feature_mean, beta):
        N = features.size(0)
        C = self.class_num
        A = features.size(1)
    ...
    def forward(self, fc, features, y_s, labels, alpha, weights, cv, manner, kg, out_new, feature_mean, beta, head):
        ...
        cv_var_new = torch.matmul(kg[head:], cv_matrix_temp).view(tail, 2048, -1)
        cv_var = cv_var.cuda()
        ...
        fc_kg = list(fc.parameters())[0]
        fc_kg_new = fc_kg

        aug_y = self.FSDA(fc_kg_new, features, y_s, labels, cv, alpha, kg, out_new, feature_mean, beta)
